...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!hasAccess(id, auth.userId)) return NextResponse.json({ error: 'Not found' }, { status: 404 });

  const item = db.prepare(`SELECT i.*, ${LABEL_SUBQUERY} FROM items i WHERE i.id = ?`).get(id);
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!auth) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  const { id } = await params;
  const item = db.prepare('SELECT id FROM items WHERE id = ? AND user_id = ?').get(id, auth.userId);
