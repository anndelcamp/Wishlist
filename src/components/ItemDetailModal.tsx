...
export default function ItemDetailModal({ item, labels, user, onClose, onRefreshPrice, onDelete, onUpdateLabels }: Props) {
  ...
  const handleRefreshPrice = async () => { setRefreshing(true); await onRefreshPrice(item.id); setRefreshing(false); };
              ...
              <div>
                ...
                {editingLabels ? (
                  <div className="flex flex-wrap gap-2">
                    {labels.map(label => (
                      <button key={label.id} onClick={() => togglePendingLabel(label.id)}
                    ...
                    {labels.length === 0 && <span className="text-sm text-gray-400">No wishlists yet</span>}
                  </div>
                ) : (
                  <div className="flex flex-wrap gap-2">
                    {item.labels.length > 0 ? item.labels.map(label => (
                      <span key={label.id} className="px-3 py-1 rounded-full text-sm text-white font-medium"
