    ...
    <div className="w-64 bg-gray-900 text-white flex flex-col h-full shrink-0">
              ...
              <div key={label.id} className="group relative">
                ...
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => onShareLabel(label)}
                    className="p-1 rounded text-gray-500 hover:text-indigo-300" title="Share">
                    <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
                    </svg>
                  </button>
                  <button onClick={() => { if (window.confirm(`Delete "${label.name}"? Items will not be deleted.`)) onDeleteLabel(label.id); }}
                    className="p-1 rounded text-gray-500 hover:text-red-400" title="Delete">
                    <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="p-3 border-t border-gray-700 space-y-1">
        ...
        <div className="flex items-center justify-between px-3 py-2">
          ...
          <button onClick={onLogout} className="text-xs text-gray-500 hover:text-red-400 ml-2 shrink-0">Sign out</button>
        </div>
      </div>
    </div>
  );
